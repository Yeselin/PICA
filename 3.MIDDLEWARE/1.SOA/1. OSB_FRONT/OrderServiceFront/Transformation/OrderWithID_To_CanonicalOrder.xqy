...
declare function local:func($InputBody as element() (:: schema-element(ns1:createOrderRequest) ::), 
    ...
    <ns1:createOrderRequest>
        <ns1:customer>
            <cus:id>{fn:data($InputBody/ns1:customer/cus:id)}</cus:id>
            <cus:idType>{fn:data($InputBody/ns1:customer/cus:idType)}</cus:idType>
        </ns1:customer>
        <ns1:paymentMethod>
            <ns1:paymentCode>{fn:data($InputBody/ns1:paymentMethod/ns1:paymentCode)}</ns1:paymentCode>
            <ns1:status>{fn:data($InputBody/ns1:paymentMethod/ns1:status)}</ns1:status>
        </ns1:paymentMethod>
        <ns1:orderId>{fn:data(xs:int($InputResponse/ns1:orderId))}</ns1:orderId>
        {
            for $order in $InputBody/ns1:order
            return 
            <ns1:order>
                <ns1:product>
        ...
        <ns1:orderStatus>{fn:data($InputResponse/ns1:status/ns1:status)}</ns1:orderStatus>
        <ns1:comments>{fn:data($InputResponse/ns1:status/ns1:comment)}</ns1:comments>
    </ns1:createOrderRequest>
};
